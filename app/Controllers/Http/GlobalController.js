...
class GlobalController 
{
    ...
    async execute({ request, response }) 
    {
        ...
        catch(e)
        {
            ...
            const code = await server_error.register("GlobalController.execute", JSON.stringify(data), e.toString())
    ...
    validate(name, data, script)
    {
        ...
        catch(e)
        {
            // REGISTRA O ERRO NO BANCO DE DADOS E RETORNA A REQUISIÇÃO
            const data = request.all()
            const code = await server_error.register("GlobalController.validate", JSON.stringify(data), e.toString())
            const error = {status: 400, message: "Não foi possível atender à requisição", error: code}
            response.send(error)
        }
    }
    ...
    findScript(name)
    {
        ...
        catch(e)
        {
            // REGISTRA O ERRO NO BANCO DE DADOS E RETORNA A REQUISIÇÃO
            const data = request.all()
            const code = await server_error.register("GlobalController.findScript", JSON.stringify(data), e.toString())
            const error = {status: 400, message: "Não foi possível atender à requisição", error: code}
            response.send(error)
        }
    }
