...
const scripts = require('../../../data/monsters/monsters.json')
...
class MonsterController 
{
    ...
    async execute({ request, response }) 
    {
        try
        {
            ...
            script = require(`../../../data/monsters/${script}`)
        ...
        catch(e)
        {
            ...
            const code = await server_error.register("MonsterController.execute", JSON.stringify(data), e.toString(), `${__dirname}/${__filename}`)
    ...
    validate(name, data, script)
    {
        ...
        catch(e)
        {
            // REGISTRA O ERRO NO BANCO DE DADOS E RETORNA A REQUISIÇÃO
            const data = request.all()
            const code = await server_error.register("MonsterController.validate", JSON.stringify(data), e.toString(), `${__dirname}/${__filename}`)
            const error = {status: 400, message: "Não foi possível atender à requisição", error: code}
            response.send(error)
        }
    }
    ...
    findScript(name)
    {
        ...
        catch(e)
        {
            // REGISTRA O ERRO NO BANCO DE DADOS E RETORNA A REQUISIÇÃO
            const data = request.all()
            const code = await server_error.register("MonsterController.findScript", JSON.stringify(data), e.toString(), `${__dirname}/${__filename}`)
            const error = {status: 400, message: "Não foi possível atender à requisição", error: code}
            response.send(error)
        }
    }
