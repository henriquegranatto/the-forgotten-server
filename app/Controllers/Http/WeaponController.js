...
const scripts = require('../../../data/weapons/weapons.json')
...
class WeaponController 
{
    ...
    async execute({ request, response }) 
    {
        try
        {
            ...
            script = require(`../../../data/weapons/${script}`)
        ...
        catch(e)
        {
            ...
            const code = await server_error.register("WeaponController.execute", JSON.stringify(data), e.toString(), `${__dirname}/${__filename}`)
    ...
    validate(name, data, script)
    {
        ...
        catch(e)
        {
            // REGISTRA O ERRO NO BANCO DE DADOS E RETORNA A REQUISIÇÃO
            const data = request.all()
            const code = await server_error.register("WeaponController.validate", JSON.stringify(data), e.toString(), `${__dirname}/${__filename}`)
            const error = {status: 400, message: "Não foi possível atender à requisição", error: code}
            response.send(error)
        }
    }
    ...
    findScript(name)
    {
        ...
        catch(e)
        {
            // REGISTRA O ERRO NO BANCO DE DADOS E RETORNA A REQUISIÇÃO
            const data = request.all()
            const code = await server_error.register("WeaponController.findScript", JSON.stringify(data), e.toString(), `${__dirname}/${__filename}`)
            const error = {status: 400, message: "Não foi possível atender à requisição", error: code}
            response.send(error)
        }
    }
